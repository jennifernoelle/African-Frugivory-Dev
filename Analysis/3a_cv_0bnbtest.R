...
# In this version we keep the baboons in, but don't use any of their observations 
# for CV
# We want to see if keeping baboons in worsens performance on non-baboon pairs
...
# Create alternate expert-defined occurrence: not using because this is Occurrence 1
...
# Expert 0b
obs_OP <- ifelse(obs_OP == 0.75, 0.5, obs_OP)
obs_OP <- ifelse(obs_OP == 0.5, 0.35, obs_OP)
obs_OP <- ifelse(obs_OP == 0.45, 0.25, obs_OP)
obs_OP <- ifelse(obs_OP == 0.25, 0.15, obs_OP)
...
# Set all baboon interactions to 0
# This is used only for choosing CV obs to hold out
comb_A <- apply(obs_A, c(1, 2), sum)
comb_A <- (comb_A > 0) * 1
use_comb_A <- comb_A
use_comb_A[-wh_keep_m, ] <- 0
...
mcmc.cv.parallel <- function(rr, n.cv, 
  ...
  # In this version we do not use baboons for any of the held-out interactions
  # Their interactions are zeroed out in use_comb_A 
  set_out <- matrix(0, nrow = nM, ncol = nP)
  set_out[sample(which(use_comb_A ==1), n.cv)] <- 1
