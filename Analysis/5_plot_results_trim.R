...
p_is <- array(NA, dim = c(nchains, nM))
p_js <- array(NA, dim = c(nchains, nP))
for (ii in 1 : nchains) {
  ...
  p_is[ii, ] <- all_res[[ii]]$p_detect[[1]] # mammal detection probabilities
  p_js[ii, ] <- all_res[[ii]]$p_detect[[2]] # plant detection probabilities
}
...
hist(p_is, main = "Animals Detection Probabilities \n with Expert Occ 3", xlab = "Posterior Mean")
abline(v = mean(p_is))
#dev.off()

#png(file = paste0(results_path, "Pi_j_", date, ".png"))
hist(p_js, main = "Plants Detection Probabilities \n with Expert Occ 3", xlab = "Posterior Mean")
abline(v = mean(p_js))
#dev.off()
