...
obs_OP <- ifelse(obs_OP == 0.5, 0.25, obs_OP) 
obs_OP <- ifelse(obs_OP == 0.25, 0.15, obs_OP) 

# Modifications for species with only one study, at a very isolated and floristically unique location
these_mammals <- c("Cercocebus_atys", "Erythrocebus_patas", "Papio_anubis", "Papio_cyanocephalus")
these_ids <- c(44,19, 52,33)
these_studies <- paste0("Study.", these_ids)

# Boost occurrence probabilities in isolated studies
obs_OP <- ifelse(colnames(obs_OP) %in% these_studies & obs_OP == 0.15, 0.35, obs_OP)
obs_OP <- ifelse(colnames(obs_OP) %in% these_studies & obs_OP == 0, 0.15, obs_OP)
