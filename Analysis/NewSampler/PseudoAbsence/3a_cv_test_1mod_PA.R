...
obs_OPb <- ifelse(obs_OP == 1, 1, 0) # was previously 0.5
...
comb_OF <- (comb_OF > 1)*1 # Species with at least 2 observable interaction opportunities, was previously 5 but used different binary conversion
...
sum(comb_A_noint == -1) # only 104, so maybe only holdout 25
