...
traits.p <- traits.p.709[traits.p.709$Species %in% common.plants, ]

# # Subset plants to simplify for exploratory analysis
# # Issue: one Erythrocebus patas only eats Celtis toka and nothing else eats Celtis_toka
# # Remove that study too
# apply(obs_A, 2, sum) # note sum plants have no observed interactions but they were present, this is fine
# common.plants <- names(which(apply(obs_A, 2, sum)>3))
# 
...
# good.mammals <- names(which(apply(obs_A, 1, sum)>1))
# 
# obs_A <- obs_A[unlist(dimnames(obs_A)[1]) %in% good.mammals, unlist(dimnames(obs_A)[2]) %in% common.plants, good.studies ]
# obs_OP <- obs_OP[rownames(obs_OP) %in% common.plants, ]
# obs_OM <- obs_OM[rownames(obs_OM) %in% good.mammals,]
# F_obs <- F_obs_m[rownames(obs_OM) %in% good.mammals, colnames(F_obs_m) %in% common.plants, good.studies]
# Cu <- Cu[rownames(Cu) %in% good.mammals, colnames(Cu) %in% good.mammals]
# Cv <- Cv[rownames(Cv) %in% common.plants, colnames(Cv) %in% common.plants]
# obs_W <- Obs_W[rownames(Obs_W) %in% common.plants, ]
# obs_X <- Obs_X[rownames(Obs_X) %in% good.mammals, ]
...
# Accuracy check: this doesn't totally make sense because we don't think zeros are necessarily zeros
# But we can look at the false negatives 
...
# Superheat  sometimes fails due to all values being too close to 1
...
# Phylogenetic correlation was previously more important for plants than animals
