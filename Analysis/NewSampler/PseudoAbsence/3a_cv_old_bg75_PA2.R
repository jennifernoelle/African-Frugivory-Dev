...
### SET UP HELDOUT OBSERVED INTERACTIONS
...
# Note that due to seasonality of fruit, etc., the same fruit may not always be present at a site
...
# Define psuedoabsence: pairs with at least three observable interaction but no interactions observed
comb_A_neg <- ifelse(comb_A == 0, -1, comb_A)
comb_A_noint <- comb_A_neg * comb_OF
comb_A_noint <- ifelse(comb_A_noint < -2, -1, 0)
sum(comb_A_noint == -1) # only 17 with at least three noninteractions, so we holdout 5
#View(comb_A_noint) # Pan troglodytes x Caloncaba glauca: seeds are poisonous, makes sense

### SET UP OCCURRENCE SCENARIO
