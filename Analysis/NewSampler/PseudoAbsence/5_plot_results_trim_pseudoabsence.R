...
date <- 'Aug3_old_bg75_PA2'
date <- 'July18_old_1mod_PA'
date <- 'Aug3_NS_1mod_PA'
...
results_path <- 'Results/NewSampler/PA/'
...
comb_OF <- (comb_OF > 5)*1 # Species with at least 5 observable interaction opportunities
...
comb_A_noint <- comb_A_neg * comb_OF # is -1 for observable noninteractions

# --------------- Cross validation results ----------------- #

# Getting the results together (held out indicies and predictions)
all_indices1 <- array(NA, dim = c(repetitions, n.cv1, 2))
all_indices2 <- array(NA, dim = c(repetitions, n.cv2, 2))
our_preds <- array(NA, dim = c(repetitions, nM, nP))

# Getting all predictions together
for (rr in 1 : repetitions) {
  cv_indices1 <- loadRData(paste0(results_path, 'cv_indices1_', date, '_', rr, '.dat'))
  cv_indices2 <- loadRData(paste0(results_path, 'cv_indices2_', date, '_', rr, '.dat'))
  pred <- loadRData(paste0(results_path, 'pred_', date, '_', rr, '.dat'))
  all_indices1[rr, , ] <- cv_indices1
  all_indices2[rr, , ] <- cv_indices2
  our_preds[rr, , ] <- pred # all predictions
}


# Predictions of the held out known interactions from model: 100 indices held out each time - always interactions
pred1 <- array(NA, dim = c(repetitions, n.cv1))
for (rr in 1 : repetitions) {
  for (ii in 1 : n.cv1) {
    pred1[rr, ii] <- our_preds[rr, all_indices1[rr, ii, 1], all_indices1[rr, ii, 2]] # just the cv1 preds
  }
}

# Predictions of the held out non-interactions from model: 50 indices held out each time - always noninteractions
pred2 <- array(NA, dim = c(repetitions, n.cv2))
for (rr in 1 : repetitions) {
  for (ii in 1 : n.cv2) {
    pred2[rr, ii] <- our_preds[rr, all_indices2[rr, ii, 1], all_indices2[rr, ii, 2]] # just the cv2 preds
  }
}
...
pred_mean1 <- apply(pred1, 1, mean)
pred_median1 <- apply(pred1, 1, median)
pred_mean2 <- apply(pred2, 1, mean)
pred_median2 <- apply(pred2, 1, median)


# Pseudo precision: 
mean(pred_mean1/overall_mean)
mean(pred_median1/overall_median)
mean(pred_mean2/overall_mean)
mean(pred_median2/overall_median)

# Pseudo accuracy: 
mean(pred_mean1)
sum(pred1>0.5)/length(pred1) # what proportion of true interactions are predicted as "likely" > 0.5
sum(pred1>0.75)/length(pred1) # what proportion of true interactions are predicted as "v likely" >0.75

mean(pred_mean2) 
sum(pred2<0.5)/length(pred2) # what proportion of non-interactions are predicted as "unlikely" <0.5; 
sum(pred2<0.25)/length(pred2) # what proportion of non-interactions are predicted as "v unlikely" <0.25;

apply(all_indices2, 1, function(x) sum(comb_A[x]))
