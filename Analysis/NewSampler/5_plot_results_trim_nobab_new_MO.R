...
for (ii in 1 : nchains) {
  cat("\n Chain = ", ii)
  res <- loadRData(paste0(results_path, date, '/res_',  date, '_MO_', ii, '.dat')) 
...
# Why does Colobus polykomos eat everything?

monkey <- pred_ours[,which(colnames(pred_ours)=="Colobus_polykomos"), ]
apply(monkey, 2, function(x) hist(x))
summary(monkey[,1])

obs_A[rownames(obs_A) == "Colobus_polykomos",,13] # only occurs in one study
monkey <- obs_A[rownames(obs_A) == "Colobus_polykomos",,13]
OP_monkey <- obs_OP[, 13]
sum(OP_monkey ==1)
table(OP_monkey)
mean(monkey[OP_monkey ==1])
