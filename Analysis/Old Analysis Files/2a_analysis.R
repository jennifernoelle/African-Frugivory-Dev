...
source(paste0(source_path, 'MCMC_function_fixRho.R'))
source(paste0(source_path, 'MCMC_function_fixRhoTau.R'))
...
burn <- 2000 # original 40000, reasonable 2500
thin <- 5 # original 40
...
for (cc in 1 : nchains) {  # Chain index:
  ...
  # Running the method:
  mcmc <- MCMC.rho.tau.Fixed(obs_A = obs_A, focus = F_obs_m, occur_B = obs_OM, occur_P = obs_OP,
