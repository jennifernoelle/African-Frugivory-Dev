...
# Note that we are only saving one version of the occurrence matrices
# which have the convention, probs can be changed downstream as desired
  # 1 = study occurrence
  # 0.75 = site occurrence
  # 0.5 = country occurrence
  # 0 else
...
# Fix Study 13: maybe it's actually fine as-is: there are multiple plants not just the one 
# Supposed to be only Gorilla gorilla and Cola lizae but there are many plants listed actually
# s.13.index <- which(s.ids ==13)
# mammal.13 <- which(rowSums(A.obs.m[,,s.13.index])>0) 
# plant.13 <- which(colSums(A.obs.m[,,s.13.index])>0)
# obs_F[,,which(s.ids==13)] <- 0
