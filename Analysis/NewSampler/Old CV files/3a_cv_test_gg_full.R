...
date <- 'July14_NewSampler_gg_full'
...
## Improved guess
# Same study: 1, same site: 0.75 -> 0.9
# Same country and habitat: 0.5 -> 0.8, same region and habitat: 0.45 -> 0.3, same habitat only: 0.25 -> 0.2,
# Same country not habitat: 0.1, same region not habitat: 0.05 -> 0.01

obs_OP <- ifelse(obs_OP == 0.75, 0.9, obs_OP)
obs_OP <- ifelse(obs_OP == 0.5, 0.8, obs_OP)
obs_OP <- ifelse(obs_OP == 0.45, 0.3, obs_OP)
...
mh_n_pjs <- 70
mh_n_rho <- 100
mh_occ_sd <-  0.25 #0.175 #0.25 # increase prior variance if weak guess is used
mh_occ_step <- 0.25 #0.25 
...
mcmc.cv.parallel <- function(rr, n.cv, 
  ...
  mcmc <- MCMC.trim.new(obs_A = use_A, focus = use_F, p_occur_B = obs_OM, p_occur_P = obs_OP,
  ...
  # Saving the sampled occurrence probabilities
  p_occ <- list(mcmc$p_OPs, mcmc$p_OP_accepted)
  save(p_occ, file = paste0(result_path, 'p_occ_', date, '_', rr, '.dat'))
  rm(p_occ)
