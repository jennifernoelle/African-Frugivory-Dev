...
UpdExtraVar <- function(mod_coef, shr_var, prior_spec) {
  ...
  # New rate parameter:
  #if (class(mod_coef) == 'numeric') { ORIRINAL CODE
  if ('numeric' %in% class(mod_coef)){ ## ERROR 2: change to this if we're getting a fatal error  
  ...
  #if (class(mod_coef) == 'matrix') { # ORIGINAL CODE
  if('matrix' %in% class(mod_coef)){ # ERROR 2: add this if getting an error
