...
# Remove plants which only occur in the baboon studies and further subset for common plants
baboon.plants <- colnames(obs_A)[which(apply(obs_A[,, wh_keep_s], 2, sum) ==0)]
common.plants <- names(which(apply(obs_A, 2, sum)>10))
wh_keep_p <- which(!(p.names %in% baboon.plants) & p.names %in% common.plants)
